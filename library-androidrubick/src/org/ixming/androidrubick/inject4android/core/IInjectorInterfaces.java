package org.ixming.androidrubick.inject4android.core;
 ...
 *
 */
public interface IInjectorInterfaces extends IViewInjector, IResInjector, IOnClickListenerInjector { }
...
interface IViewInjector {
	...
	/**
	 * 实现动态获取并注入View。
	 * @return 如果field中存在{@link ViewById}该标注，则返回TRUE。
	 */
...
interface IResInjector {
	...
	/**
	 * 实现动态获取并注入资源。
	 * @return 如果field中存在{@link ResById}该标注，则返回TRUE。
	 */
...
//	boolean injectViewOnClickListener(Object target, Method method);
	...
	/**
	 * 实现动态获取并注入单击事件监听器。
	 * @return 如果method中存在 {@link OnClick} 标注，则返回TRUE。
	 */
